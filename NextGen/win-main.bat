...
docker run --rm --entrypoint=deploy-win.bat -v "%~dp0container-scripts":C:/container-scripts -v "%cd%\cert":C:\cert -w /container-scripts -e APSIM_CERT=C:\cert\apsim.p12 -e ghprbPullId -e PULL_ID -e APSIM_SITE_CREDS -e APSIM_CERT_PWD -e APSIM_SITE_CREDS %container%
